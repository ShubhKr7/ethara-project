...
export default function SignupPage() {
  ...
  return (
    <div
      ...
      style={{ background: "radial-gradient(ellipse at 20% 50%, var(--auth-bg-1) 0%, var(--auth-bg-2) 40%, var(--auth-bg-3) 100%)" }}
    >
      {/* Animated ambient orbs */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <motion.div {...fadeIn(0)} className="absolute -top-32 -left-32 w-[500px] h-[500px] rounded-full blur-[120px]" style={{ background: "radial-gradient(circle, var(--auth-orb-1), transparent 70%)", animation: "orbPulse 8s ease-in-out infinite" }} />
        <motion.div {...fadeIn(0.3)} className="absolute -bottom-48 -right-24 w-[600px] h-[600px] rounded-full blur-[140px]" style={{ background: "radial-gradient(circle, var(--auth-orb-2), transparent 70%)", animation: "orbPulse 10s ease-in-out infinite 2s" }} />
        <motion.div {...fadeIn(0.6)} className="absolute top-1/4 left-1/3 w-[300px] h-[300px] rounded-full blur-[80px]" style={{ background: "radial-gradient(circle, var(--auth-orb-3), transparent 70%)", animation: "orbPulse 12s ease-in-out infinite 4s" }} />
      </div>
      ...
      <div className="relative z-10 w-full max-w-[420px]">
        {/* Card glow */}
        <motion.div {...fadeIn(0.2)} className="absolute -inset-px rounded-[1.75rem] opacity-50 blur-[2px]" style={{ background: "linear-gradient(135deg, rgba(124,58,237,0.4) 0%, rgba(37,99,235,0.2) 50%, rgba(6,182,212,0.3) 100%)" }} />
        ...
        >
          ...
          <div className="text-center mb-8">
            <motion.div {...fadeUp(0.3)} className="inline-flex items-center gap-2 bg-foreground/5 border border-foreground/10 rounded-full px-3 py-1 mb-5">
              ...
              <span className="text-foreground/60 text-[11px] font-body tracking-widest uppercase">Join the Vision</span>
            </motion.div>
            ...
            <motion.p {...fadeUp(0.44)} className="text-foreground/50 text-sm font-body font-light">
              Start your journey with Ethara today
            </motion.p>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <motion.div {...fadeUp(0.5)} className="space-y-1.5">
              <label className="block text-[11px] font-body font-semibold text-foreground/50 uppercase tracking-[0.1em]">Full Name</label>
              <input
                id="name" name="name" type="text" required
                className="block w-full rounded-xl border border-foreground/10 bg-foreground/5 px-4 py-3 text-sm text-foreground placeholder-foreground/25 focus:border-violet-500/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20 transition-all font-body"
                placeholder="Ethara User"
              />
            </motion.div>
            ...
            <motion.div {...fadeUp(0.62)} className="space-y-1.5">
            ...
            >
              ...
              <span className="relative">{loading ? "Registering…" : "Register Now"}</span>
              {!loading && (
          ...
          <motion.div {...fadeUp(0.74)} className="flex items-center gap-3 my-6">
            <div className="flex-1 h-px bg-foreground/10" />
            <span className="text-foreground/30 text-xs font-body">or</span>
            <div className="flex-1 h-px bg-foreground/10" />
          </motion.div>

          <motion.p {...fadeUp(0.78)} className="text-center text-sm text-foreground/40 font-body">
            Already have an account?{" "}
            <Link href="/login" className="text-violet-500 hover:text-violet-600 transition-colors font-medium">
              Sign in →
            </Link>
          </motion.p>
        </motion.div>
